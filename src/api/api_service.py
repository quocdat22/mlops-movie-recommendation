...
class RecommendationRequest(BaseModel):
    ...
    exclude_same_year: bool = Field(
        False, description="Exclude movies from the same year"
    )
...
class BatchRecommendationRequest(BaseModel):
    ...
    top_k: int = Field(
        10, ge=1, le=50, description="Number of recommendations per movie"
    )
    min_similarity: float = Field(
        0.0, ge=0.0, le=1.0, description="Minimum similarity threshold"
    )
...
class SearchRequest(BaseModel):
    query: str = Field(
        ..., min_length=3, description="Text query describing movie features"
    )
    top_k: int = Field(
        10, ge=1, le=50, description="Number of similar movies to return"
    )
...
async def lifespan(app: FastAPI):
    ...
    if not model_dir.exists():
        ...
        if success:
            ...
            logger.info(
                f"Model loaded successfully with {stats['total_movies']:,} movies"
            )
    ...
    try:
        ...
        return {
            "model_status": {
                ...
                "vocabulary_size": int(
                    get_metric_value(metrics.model_vocabulary_size, 0)
                ),
            },
            "cache_metrics": {
            ...
            "request_metrics": {
                "recommendations_generated": int(
                    get_metric_value(metrics.recommendations_generated_total, 0)
                ),
                "search_queries": int(
                    get_metric_value(metrics.search_queries_total, 0)
                ),
        ...
        return {
            ...
            "cache_metrics": {"hits": 0, "misses": 0, "current_size": 0},
            "request_metrics": {
...
async def get_recommendations(request: RecommendationRequest):
    ...
    try:
        # Record request size
        metrics.recommendation_request_size.labels(endpoint="recommendations").observe(
            request.top_k
        )
        ...
        metrics.record_recommendations_generated(
            "recommendations", len(recommendations), "success"
        )
...
async def get_recommendations_get(
    ...
    min_similarity: float = Query(
        0.0, ge=0.0, le=1.0, description="Minimum similarity"
    ),
...
async def get_batch_recommendations(request: BatchRecommendationRequest):
    ...
    if len(request.movie_ids) > 20:
        raise HTTPException(
            status_code=400, detail="Maximum 20 movies per batch request"
        )
    ...
    try:
        ...
        def search():
            return recommender.search_movies(
                query=request.query, top_k=request.top_k, search_type="hybrid"
            )
