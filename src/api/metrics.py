...
class RecommendationMetrics:
    ...
    def __init__(self):
        ...
        self.cache_hits_total = Counter(
            "recommendation_cache_hits_total", "Total number of cache hits"
        )

        self.cache_misses_total = Counter(
            "recommendation_cache_misses_total", "Total number of cache misses"
        )

        self.cache_size = Gauge(
            "recommendation_cache_size", "Current number of items in cache"
        )
        ...
        self.model_total_movies = Gauge(
            "recommendation_model_total_movies", "Total number of movies in the model"
        )

        self.model_vocabulary_size = Gauge(
            "recommendation_model_vocabulary_size", "Size of the TF-IDF vocabulary"
        )
            ...
            def wrapper(*args, **kwargs) -> Any:
                ...
                try:
                    ...
                    self.model_inference_duration.labels(
                        endpoint=endpoint, movie_count=str(movie_count)
                    ).observe(duration)
                    return result
    ...
    def record_recommendations_generated(
        self, endpoint: str, count: int, status: str = "success"
    ):
    ...
    def update_model_status(
        self, is_loaded: bool, total_movies: int = 0, vocab_size: int = 0
    ):
