...
class MovieRecommendationAPIClient:
    ...
    def get_recommendations(self, movie_id: int, top_k: int = 10, 
                          ...
                          exclude_same_year: bool = False) -> Dict:
    ...
    def get_batch_recommendations(self, movie_ids: List[int], 
        ...
        response = self.session.post(f"{self.base_url}/recommendations/batch", json=payload)
        response.raise_for_status()
        return response.json()
...
def main():
    ...
    try:
        ...
        if not health.get('model_loaded'):
            print("ERROR: Model not loaded. Please start the API server with a trained model.")
            return
        ...
        if popular:
            ...
            for i, rec in enumerate(recommendations['recommendations'][:3], 1):
                print(f"{i}. {rec['title']} (Similarity: {rec.get('similarity', 'N/A')})")
