...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">🎬 MLOps Movie Recommendation Dashboard</h1>', unsafe_allow_html=True)
    
    # Sidebar
    st.sidebar.title("Navigation")
    page = st.sidebar.selectbox(
        "Choose a page",
        ["🏠 Overview", "📊 Model Monitoring", "🔧 Pipeline Management", "🧪 Testing", "📈 Analytics", "⚙️ System"]
    )
...
def show_model_monitoring(dashboard):
    ...
    with col1:
        if rec_success:
            st.metric("Recommendation Response", f"{rec_result['response_time_ms']:.1f}ms", delta="Fast")
            st.success(f"✅ Generated {len(rec_result['recommendations'])} recommendations")
        else:
    ...
    with col2:
        if search_success:
            st.metric("Search Response", f"{search_result['response_time_ms']:.1f}ms", delta="Fast")
    ...
    with col3:
        if model_stats:
            ...
            st.info(f"Matrix: {model_stats['matrix_shape'][0]}×{model_stats['matrix_shape'][1]}")
        else:
...
def show_pipeline_management(dashboard):
    ...
    processed_data_path = dashboard.project_root / "data" / "processed" / "processed_movies.parquet"
    model_path = dashboard.project_root / "models" / "content_based" / "similarity_matrix.npy"
    ...
    schedules = [
        ...
        {"name": "Weekly Model Retrain", "status": "Active", "next_run": "2025-07-06 02:00"},
        {"name": "Hourly Health Check", "status": "Active", "next_run": "2025-06-30 06:00"}
    ]
                ...
                if success:
                    ...
                    if recs:
                        ...
                        for i, rec in enumerate(recs, 1):
                            st.write(f"{i}. {rec['title']} (Similarity: {rec['similarity']:.3f})")
                    else:
                ...
                if success:
                    ...
                    if results:
                        ...
                        for i, movie in enumerate(results, 1):
                            st.write(f"{i}. {movie['title']} (Similarity: {movie['similarity']:.3f})")
                    else:
        ...
        with st.spinner("Running comprehensive tests..."):
            ...
            for movie_id in test_movies:
                ...
                results.append({
                    'movie_id': movie_id,
                    'success': success,
                    'response_time': result.get('response_time_ms', 0) if success else 0,
                    'num_recommendations': len(result.get('recommendations', [])) if success else 0
                })
            ...
            with col3:
                avg_recs = df_results[df_results['success']]['num_recommendations'].mean()
...
def show_analytics(dashboard):
    ...
    with col2:
        fig = px.line(df_analytics, x='date', y='response_time', title="Average Response Time")
        fig.update_layout(yaxis_title="Response Time (ms)")
        st.plotly_chart(fig, use_container_width=True)
...
def show_system_metrics(dashboard):
    ...
    with col2:
        mem_color = "red" if system_metrics['memory_percent'] > 80 else "orange" if system_metrics['memory_percent'] > 60 else "green"
        st.metric("Memory Usage", f"{system_metrics['memory_percent']:.1f}%")
        st.progress(system_metrics['memory_percent'] / 100)
    
    with col3:
        disk_color = "red" if system_metrics['disk_percent'] > 90 else "orange" if system_metrics['disk_percent'] > 70 else "green"
