...
def process_and_insert_recent_movies(engine, json_file_path):
    ...
    try:
        ...
        columns_to_keep = ['id', 'adult', 'backdrop_path', 'genre_ids', 'original_language', 
        ...
        with engine.connect() as connection:
            try:
                ...
                existing_ids = existing_ids_df['id'].tolist() if not existing_ids_df.empty else []
            except Exception:
        ...
        if not new_movies_df.empty:
            logging.info(f"Tìm thấy {len(new_movies_df)} phim mới cần chèn vào database.")
            ...
            logging.info(f"Đã chèn thành công {len(new_movies_df)} phim mới vào database.")
            return len(new_movies_df)
        else:
            logging.info("Không có phim mới nào để chèn (tất cả đã tồn tại trong database).")
            return 0
...
def main():
    ...
    try:
        ...
        new_movies_added = process_and_insert_recent_movies(db_engine, recent_movies_file)
        ...
        logging.info(f"Task 2 hoàn thành: Đã thêm {new_movies_added} phim mới. Tổng số phim trong database: {total_movies}")
