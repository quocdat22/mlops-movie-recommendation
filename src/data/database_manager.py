...
class DatabaseManager:
    ...
    def insert_movie(self, movie_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error inserting movie {movie_data.get('id', 'unknown')}: {e}"
            )
            return False

    def batch_insert_movies(
        self, movies_data: List[Dict[str, Any]], batch_size: int = 100
    ):
        ...
        logger.info(
            f"Starting to insert {total_movies} movies in batches of {batch_size}"
        )

        for i in range(0, total_movies, batch_size):
            batch = movies_data[i : i + batch_size]
            logger.info(
                f"Processing batch {i//batch_size + 1}/{(total_movies + batch_size - 1)//batch_size}"
            )
        ...
        logger.info(
            f"Insertion completed. Inserted: {inserted_count}, Errors: {error_count}"
        )
