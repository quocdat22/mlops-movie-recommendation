...
def clean_missing_values(df):
    ...
    if null_counts.sum() > 0:
        logging.warning(
            f"Còn lại các giá trị null: {null_counts[null_counts > 0].to_dict()}"
        )
    else:
...
def standardize_data(df):
    ...
    df["cast_clean"] = df["cast_parsed"].apply(
        lambda x: " ".join(extract_names_robust(x, 5))
    )
    df["keywords_clean"] = df["keywords_parsed"].apply(
        lambda x: " ".join(extract_names_robust(x, 10))
    )
...
def create_tags_feature(df):
    ...
    logging.info(
        f"Đã tạo tags cho {len(df)} phim. {empty_tags.sum()} phim có tags rỗng."
    )
...
def save_processed_data(df, output_path):
    ...
    try:
        ...
        try:
            ...
            logging.info(
                f"Đã lưu {len(processed_df)} phim đã xử lý vào CSV: {csv_path}"
            )
...
def generate_preprocessing_report(df, output_path):
    ...
    try:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"Phim có overview: {has_overview}/{total_movies} ({has_overview/total_movies*100:.1f}%)\n"
            )
            f.write(
                f"Phim có cast info: {has_cast}/{total_movies} ({has_cast/total_movies*100:.1f}%)\n"
            )
            f.write(
                f"Phim có keywords: {has_keywords}/{total_movies} ({has_keywords/total_movies*100:.1f}%)\n"
            )
            f.write(
                f"Phim có tags: {has_tags}/{total_movies} ({has_tags/total_movies*100:.1f}%)\n\n"
            )

            f.write("=== THỐNG KÊ TAGS ===\n")
            f.write(
                f"Độ dài trung bình tags: {df['tags'].str.len().mean():.1f} ký tự\n"
            )
