...
class ContentBasedRecommender:
                ...
                for raw_file in raw_data_files:
                    if raw_file.exists():
                        try:
                            ...
                            logger.warning(f"Error loading raw data from {raw_file}: {e}")
            else:
                try:
    ...
    def get_movie_info(self, movie_id: int) -> Optional[Dict]:
        ...
        if self.raw_movies_data and movie_id in self.raw_movies_data:
            ...
            result.update({
                ...
                'genres': ', '.join([str(g) for g in raw_movie.get('genre_ids', [])]) if raw_movie.get('genre_ids') else '',
    ...
    def get_recommendations(
        ...
        for idx, score in sim_scores:
            ...
            if exclude_same_year and source_movie:
                ...
                if (rec_movie and source_movie.get('release_date') and 
                    rec_movie.get('release_date')):
    ...
    def search_similar_movies(
        ...
        try:
            ...
            similarities = np.array(query_vector.dot(self.tfidf_matrix.T).toarray()).flatten()
    ...
    def search_movies(self, query: str, top_k: int = 10, search_type: str = 'hybrid') -> List[Dict]:
        """
...
def main():
    ...
    for i, movie in enumerate(popular, 1):
        print(f"{i}. {movie['title']} ({movie['release_date'][:4] if movie['release_date'] else 'N/A'}) - {movie['vote_average']:.1f}")
    ...
    if popular:
        ...
        recommendations = recommender.get_recommendations(demo_movie['movie_id'], top_k=5)
        for i, rec in enumerate(recommendations, 1):
            print(f"{i}. {rec['title']} ({rec['release_date'][:4] if rec['release_date'] else 'N/A'}) - "
                  f"Similarity: {rec['similarity']:.3f}")
    
    # Demo text-based search
    print(f"\nSimilar movies to 'action superhero adventure':")
    text_recommendations = recommender.search_similar_movies("action superhero adventure", top_k=5)
    ...
    combined_recommendations = recommender.search_movies("superhero", top_k=5, search_type='hybrid')
