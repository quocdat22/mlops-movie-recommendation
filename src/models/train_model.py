...
class ContentBasedTrainer:
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            movies_with_tags = len(self.movies_df[self.movies_df['tags'].notna() & 
                                                 (self.movies_df['tags'] != '')])
            tag_coverage = (movies_with_tags / total_movies) * 100
            
            logger.info(f"Data quality: {movies_with_tags}/{total_movies} movies have tags ({tag_coverage:.1f}%)")
            
            # Filter out movies without tags for training
            original_count = len(self.movies_df)
            self.movies_df = self.movies_df[
                self.movies_df['tags'].notna() & 
                (self.movies_df['tags'] != '')
            ].copy()
            filtered_count = len(self.movies_df)
            
            logger.info(f"Filtered dataset: {filtered_count}/{original_count} movies for training")
            
            # Create movie mappings
            self.movie_to_idx = {movie_id: idx for idx, movie_id in enumerate(self.movies_df['movie_id'])}
    ...
    def vectorize_features(self) -> sp.csr_matrix:
        ...
        logger.info(f"Matrix sparsity: {(1 - self.tfidf_matrix.nnz / (self.tfidf_matrix.shape[0] * self.tfidf_matrix.shape[1])) * 100:.2f}%")
    ...
    def compute_similarity_matrix(self) -> np.ndarray:
        ...
        non_diag_similarities = self.similarity_matrix[np.triu_indices_from(self.similarity_matrix, k=1)]
        logger.info(f"Similarity stats - Mean: {non_diag_similarities.mean():.4f}, "
    ...
    def validate_model(self) -> Dict:
        ...
        for _, movie in test_movies.iterrows():
            ...
            if recs:
                ...
                logger.info(f"Sample recommendations for '{title}': {[r['title'] for r in recs[:3]]}")
        
        # Calculate validation metrics
        if recommendations_quality:
            validation_metrics = {
                'total_tested': len(recommendations_quality),
                'avg_recommendations_per_movie': np.mean([r['recommendations'] for r in recommendations_quality]),
                'avg_similarity_score': np.mean([r['avg_similarity'] for r in recommendations_quality]),
                'diversity_rate': np.mean([r['diverse'] for r in recommendations_quality]),
                'sample_tests': recommendations_quality[:3]  # Store first 3 for reporting
            }
    ...
    def get_recommendations(self, movie_id: int, top_k: int = 10) -> List[Dict]:
        ...
        for idx, score in sim_scores[:top_k]:
            ...
            movie_info = self.movies_df[self.movies_df['movie_id'] == rec_movie_id].iloc[0]
    ...
    def generate_training_report(self, validation_metrics: Dict, artifacts: Dict[str, str]) -> str:
...
"""
        for artifact_type, path in artifacts.items():
            ...
            report_content += f"- {artifact_type.title()}: {path} ({file_size:.1f} MB)\n"
...
"""
    ...
    def train(self) -> Dict:
        ...
        try:
            ...
            logger.info(f"Model ready for deployment with {len(self.movies_df):,} movies")
...
def main():
    ...
    try:
        ...
        if results['validation_metrics'] and 'sample_tests' in results['validation_metrics']:
            print("\nSample Recommendations:")
