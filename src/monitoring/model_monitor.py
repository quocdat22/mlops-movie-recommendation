...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("model_monitoring.log"), logging.StreamHandler()],
)
...
class ModelMonitor:
        ...
        for movie_id in self.test_movie_ids:
            try:
                ...
                if response.status_code == 200:
                    ...
                    avg_similarity = (
                        np.mean([r["similarity"] for r in recommendations])
                        if recommendations
                        else 0
                    )
    ...
    def test_search_functionality(self) -> Dict:
        ...
        if successful_searches:
            avg_search_time = np.mean(
                [r["response_time_ms"] for r in successful_searches]
            )
    ...
    def log_summary(self, results: Dict):
        ...
        logger.info(
            f"API Health: {health['status']} ({health['response_time_ms']:.1f}ms)"
        )
        logger.info(
            f"Recommendations: {rec_test['success_rate']:.2%} success, {rec_test['avg_response_time_ms']:.1f}ms avg"
        )
        logger.info(
            f"Search: {search_test['search_success_rate']:.2%} success, {search_test['avg_search_time_ms']:.1f}ms avg"
        )
